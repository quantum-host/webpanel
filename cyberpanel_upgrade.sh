...
Pre_Upgrade_Setup_Git_URL() {
  if [[ $Server_Country != "CN" ]] ; then
    Git_User="quantum-host"
    Git_Content_URL="https://raw.githubusercontent.com/${Git_User}/webpanel"
    Git_Clone_URL="https://github.com/${Git_User}/webpanel.git"
  else
    Git_User="quantum-host"
...
if [[ "$Server_Country" = "CN" ]] ; then
  sed -i 's|git clone https://github.com/quantum-host/webpanel|echo git cloned|g' upgrade.py

  Retry_Command "git clone ${Git_Clone_URL}"
    Check_Return "git clone ${Git_Clone_URL}"

  # shellcheck disable=SC2086
  sed -i 's|https://raw.githubusercontent.com/quantum-host/webpanel/stable/install/litespeed/httpd_config.xml|'${Git_Content_URL}/${Branch_Name}'//install/litespeed/httpd_config.xml|g' upgrade.py
