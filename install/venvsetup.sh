...
#line1="$(grep -n "github.com/quantum-host/webpanel" install.py | head -n 1 | cut -d: -f1)"
#line2=$((line1 - 1))
...
sed -i 's|git clone https://github.com/quantum-host/webpanel|wget https://cyberpanel.sh/cyberpanel-git.tar.gz \&\& tar xzvf cyberpanel-git.tar.gz|g' install.py
...
wget -O requirements.txt https://raw.githubusercontent.com/quantum-host/webpanel/1.8.0/requirments.txt
...
if [[ $DEV == "ON" ]] ; then
	#install dev branch 
	#wget https://raw.githubusercontent.com/quantum-host/webpanel/$BRANCH_NAME/requirments.txt
	cd /usr/local/
	python3.6 -m venv CyberPanel
	source /usr/local/CyberPanel/bin/activate
	wget -O requirements.txt https://raw.githubusercontent.com/quantum-host/webpanel/$BRANCH_NAME/requirments.txt
